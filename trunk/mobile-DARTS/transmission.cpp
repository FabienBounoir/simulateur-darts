...
void Transmission::recevoir()
{
    QByteArray donnees;
        ...
        if(trame.startsWith("$DART") && trame.endsWith("\r\n"))
        {
...
{
    qDebug() << "TRAME:" << envoyerTrame<<endl;
   ...
   /* if(portSonde->isOpen())
    ...
    if(socket->isOpen())
    {
        socket->write(envoyerTrame.toStdString().c_str());

        qDebug() << __FUNCTION__ << ": " << envoyerTrame.toStdString().c_str() << endl;
    }
