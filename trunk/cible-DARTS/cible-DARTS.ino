...
void setup()
{
  ...
  {
    pinMode(PinsLignes[ligne],INPUT);       // Définir les pin de ligne comme entrée
  ...
  pinMode(2,OUTPUT);  //Pin relié à un bouton qui permet de déclarer lorsque la fléchette est en dehors de la cible
...
void loop()
{
  if(digitalRead(interrupteur) == HIGH)
  {
    ...
    if(digitalRead(2) == HIGH) //pin utiliser pour la flechette en dehors de la cible
    {
  ...
  {
    Serial.print("Recuperer flechette\n");
    delay(2000);
  }
}
